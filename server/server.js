...
app.get("/api/cycles/:cycleId/services", authenticateUser, (req, res) => {
    ...
    db.get("SELECT id FROM cycles WHERE id = ?", [cycleId], (err, cycle) => {
        ...
        db.all("SELECT id, name, price, tip, date, cycle_id as cycleId, user_id as userId FROM services WHERE cycle_id = ? AND user_id = ? ORDER BY date DESC", [cycleId, userId], (serviceErr, services) => {
...
app.post("/api/cycles/:cycleId/services", authenticateUser, (req, res) => {
    const { cycleId } = req.params;
    const userId = req.user.id;
    const { name, price, tip, date } = req.body;
    ...
    db.get("SELECT id FROM cycles WHERE id = ?", [cycleId], (err, cycle) => {
        ...
        const sql = `INSERT INTO services (user_id, cycle_id, name, price, tip, date) VALUES (?, ?, ?, ?, ?, ?)`;
        db.run(sql, [userId, cycleId, name, parseFloat(price), parseFloat(tip || 0), date], function (serviceErr) {
            if (serviceErr) return res.status(500).json({ error: "Failed to add service: " + serviceErr.message });
            db.get("SELECT id, name, price, tip, date, cycle_id as cycleId, user_id as userId FROM services WHERE id = ?", [this.lastID], (getErr, newService) => {
...
app.put("/api/services/:serviceId", authenticateUser, (req, res) => {
    ...
    const userId = req.user.id;
    const { name, price, tip, date, cycleId } = req.body; // cycleId can be changed
    ...
    db.get("SELECT id, name, price, tip, date, cycle_id AS cycleId, user_id AS userId FROM services WHERE id = ? AND user_id = ?", [serviceId, userId], (err, service) => {
        ...
        if (cycleId && cycleId !== service.cycle_id) {
            db.get("SELECT id FROM cycles WHERE id = ?", [cycleId], (cycleErr, newCycle) => {
                ...
                performServiceUpdate(service, { name, price, tip, date, cycleId }, res);
            });
        } else {
            performServiceUpdate(service, { name, price, tip, date, cycleId: cycleId || service.cycle_id }, res);
        }
    });
});
...
function performServiceUpdate(currentService, updates, res) {
    ...
    const sql = `UPDATE services SET name = ?, price = ?, tip = ?, date = ?, cycle_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?`;
    const params = [newName, newPrice, newTip, newDate, newCycleId, currentService.id, currentService.user_id];
    db.run(sql, params, function(err) {
        ...
        db.get("SELECT id, name, price, tip, date, cycle_id as cycleId, user_id as userId FROM services WHERE id = ?", [currentService.id], (getErr, updatedService) => {
